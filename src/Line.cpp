...
namespace basicgraphics {
	...
	Line::Line(const glm::vec3 &start, const glm::vec3 &end, const glm::vec3 &normal, const float radius, const glm::vec4 &color) : _start(start), _end(end), _radius(radius), _color(color)
	{
		...
		_mesh.reset(new Mesh(textures, GL_TRIANGLE_STRIP, GL_STATIC_DRAW, cpuVertexByteSize, cpuIndexByteSize, 0, cpuVertexArray, (int)cpuIndexArray.size(), cpuIndexByteSize, &cpuIndexArray[0]));
